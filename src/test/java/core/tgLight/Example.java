...
import it.tdlight.jni.TdApi.AddLocalMessage;
...
public final class Example {
     ...
     */
    private static final TdApi.MessageSender ADMIN_ID = new TdApi.MessageSenderUser(131876907);
    ...
    private static void onUpdateNewMessage(TdApi.UpdateNewMessage update) {
        ...
        if (messageContent instanceof TdApi.MessageText messageText) {
            ...
            text[0] = messageText.text.text;
        } else {
            // We handle only text messages, the other messages will be printed as their type
            text[0] = String.format("(%s)", messageContent.getClass().getSimpleName());
        }
        ...
        client.send(new TdApi.GetChat(update.message.chatId), chatIdResult -> {
            ...
            if (Objects.equals(chatName, "cb-dev1.go.online"))
            {
                // Print the message
                System.out.printf("Received new message from chat %s: %s%n", chatName, text[0]);
            }else {
                // We handle only text messages, the other messages will be printed as their type
                text[0] = String.format("(%s)", messageContent.getClass().getSimpleName());
            }
